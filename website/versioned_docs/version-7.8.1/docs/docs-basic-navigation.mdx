...
Generally, any mobile app consists of various destinations which display some content to the user. And in the vast majority of cases using an app means navigating between these destinations. React Native Navigation provides ways for you to layout your content on the user's screen in a logical and performant manner.
...
You can specify options of each layout (Stack, component pushed into a stack, etc.) to configure various parameters like the TopBar title or color. Let's spice things up by changing the TopBar color and while we're at it, also move the title to the TopBar.

Let's change the Home screen declaration to the following and reload the app to see the changes.
```jsx
const HomeScreen = (props) => {
  return (
    <View style={styles.root}>
...
Navigation.push(props.componentId, {
  component: {
    ...
    options: { // Options object to configure the screen (not required)
      topBar: {
        title: {
...
Let's change our code to the following snippet below and reload the app. Now our Home should contain a button which when pressed, pushes a new screen into the stack. We've successfully navigated to a new screen! 👏
...
Let's run our code again - now our design is consistent across both screens.
...
In the next section we'll explore a more advance navigation pattern using BottomTabs layout and also see how, and why, multiple roots are set.
